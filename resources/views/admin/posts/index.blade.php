<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        @if (session('success'))
            <div class="bg-green-100 dark:bg-green-800 border-l-4 border-green-500 dark:border-green-400 text-green-700 dark:text-green-300 px-4 py-3 rounded-md mb-6 shadow-md"
                role="alert">
                ...
                <span class="absolute top-0 bottom-0 right-0 px-4 py-3">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                        ...
                        stroke-linejoin="round" class="lucide lucide-x-circle h-6 w-6 fill-current"
            ...
            <table
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    @foreach ($posts as $post)
                        <tr class="hover:bg-purple-50 dark:hover:bg-purple-700" data-category="{{ $post->category }}"
                            data-created-at="{{ $post->created_at->timestamp }}" data-title="{{ $post->title }}">
                            <td class="px-3 py-2">
                                {{ $post->id }}
                            </td>
                            <td class="px-3 py-2">
                            ...
                            <td class="px-3 py-2">
                                <span
                                    class="px-2 py-1 rounded-full text-xs font-semibold {{ $post->status === 'published' ? 'bg-green-100 text-green-800 dark:bg-green-700 dark:text-green-100' : ($post->status === 'draft' ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-700 dark:text-yellow-100' : 'bg-red-100 text-red-800 dark:bg-red-700 dark:text-red-100') }}">
                                    {{ $post->status }}
                                </span>
                            </td>
                            <td class="px-3 py-2">
                        ...
                        @if (count($posts) === 0)
                            <tr>
                                <td class="px-3 py-2 whitespace-nowrap text-center text-sm text-gray-500 dark:text-gray-400"
                                    colspan="6">{{ __('No posts found.') }}</td>
                            </tr>
                        @endif
                    @endforeach
                </tbody>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function filterAndSortPosts() {
                ...
                if (selectedCategory) {
                    filteredPosts = filteredPosts.filter(row => row.getAttribute('data-category') ===
                        selectedCategory);
                }
                ...
                switch (sortValue) {
                    case 'newest':
                        filteredPosts.sort((a, b) => parseInt(b.getAttribute('data-created-at')) - parseInt(a
                            .getAttribute('data-created-at')));
                        break;
                    case 'oldest':
                        filteredPosts.sort((a, b) => parseInt(a.getAttribute('data-created-at')) - parseInt(b
                            .getAttribute('data-created-at')));
                        break;
                    case 'title-asc':
                        filteredPosts.sort((a, b) => a.getAttribute('data-title').localeCompare(b.getAttribute(
                            'data-title')));
                        break;
                    case 'title-desc':
                        filteredPosts.sort((a, b) => b.getAttribute('data-title').localeCompare(a.getAttribute(
                            'data-title')));
                        break;
                }
                ...
                if (filteredPosts.length === 0) {
                    ...
                    noResultsRow.innerHTML =
                        `<td class="px-3 py-2 whitespace-nowrap text-center text-sm text-gray-500 dark:text-gray-400" colspan="6">{{ __('No posts found matching your criteria.') }}</td>`;
