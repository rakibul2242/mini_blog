        ...
        <div class="bg-white dark:bg-gray-900 shadow-xl rounded-2xl p-8 space-y-8">
            ...
            <form action="{{ route('posts.update', $post->id) }}" method="POST" class="space-y-6" enctype="multipart/form-data">
                ...
                <div class="mt-4">
                    ...
                    <input type="file" name="image" id="image" class="mt-1 w-full px-4 py-3 rounded-lg border bg-gray-200 border-gray-300 dark:border-gray-700 dark:bg-gray-800 text-gray-900 dark:text-gray-100 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-500 file:text-white hover:file:bg-blue-600 focus:ring-2 focus:ring-blue-500 focus:outline-none transition">
                    @if ($post->featured_image)
                    <img src="{{ Storage::url($post->featured_image) }}" alt="Post Image" class="mt-4 w-48 h-48 object-cover">
                    @endif
                </div>
                {{-- Content --}}
                <div>
                    ...
                    <textarea name="content" id="content" rows="8" required
                        class="mt-1 w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:ring-2 focus:ring-blue-500 focus:outline-none transition"
                        placeholder="Write your post..." aria-describedby="content-error">{{ old('content', $post->content) }}</textarea>
                    @error('content')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required
                            ...
                            <option value="{{ $statusOption['value'] }}" {{ old('status', $post->status) === $statusOption['value'] ? 'selected' : '' }}>
                                {{ $statusOption['label'] }}
                            </option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <input type="text" name="featured_image" id="featured_image"
                        ...
                        value="{{ old('featured_image', $post->featured_image) }}" aria-describedby="featured_image-error">
                    @error('featured_image')
                    <p id="featured_image-error" class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror

                    @if(old('featured_image', $post->featured_image))
                    <img src="{{ old('featured_image', $post->featured_image) }}" class="mt-4 rounded-md shadow-md max-w-full h-auto" alt="Featured Image">
                    @endif
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="created_at" id="created_at" required
                        class="mt-1 w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:outline-none transition"
                        value="{{ old('created_at', $post->created_at->format('Y-m-d')) }}" aria-describedby="created_at-error">
                    @error('created_at')
